...
class Pile {
  public:
	...
	Pile(Pile const& p) {
		...
		for(unsigned int i = 0; i < nbElementMax; i++) { data[i] = p.data[i]; }
	}
		...
		if(nbElementMax != p.nbElementMax) {
			...
			data_new == nullptr
				? throw std::runtime_error(std::string{} + __FILE__ + ":" +
										   std::to_string(__LINE__) + " ptr is null")
				: data = data_new;
		}
	...
	bool operator==(const Pile& p) const {
		...
		for(unsigned int i = 0; i < size(); i++) {
			if(p.data[i] != data[i]) return false;
		}
		return true;
	}
	...
	TYPE pop() {
		if(nbElement == 0) { THROW("nbElement == 0") }
